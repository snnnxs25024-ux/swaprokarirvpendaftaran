...
export const InputField: React.FC<InputProps> = ({ label, error, required, className = '', ...props }) => (
  <div className={`mb-4 ${className}`}>
    ...
    <input
      {...props}
      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-colors disabled:bg-gray-100 disabled:text-gray-500"
    />
...
export const TextAreaField: React.FC<TextAreaProps> = ({ label, error, required, ...props }) => (
  <div className="mb-4">
    ...
    <textarea
      {...props}
      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-colors min-h-[100px]"
    />
    {error && <p className="text-red-500 text-xs mt-1">{error}</p>}
  </div>
);
...
export const CheckboxField: React.FC<CheckboxProps> = ({ label, checked, onChange, subLabel }) => (
  <label className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-all ${checked ? 'border-brand-500 bg-brand-50' : 'border-gray-200 hover:border-gray-300'}`}>
...
export const FileUpload: React.FC<FileUploadProps> = ({ label, accept, onChange, currentFile, required }) => (
  <div className="mb-4">
    ...
    {currentFile && (
      <div className="mt-2 flex items-center gap-2 text-sm text-green-600 bg-green-50 p-2 rounded border border-green-100">
